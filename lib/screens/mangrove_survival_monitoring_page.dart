...
class _MangroveSurvivalMonitoringPageState
  ...
  /// this page (i.e. whose `activityId` points at a Mangrove Planting
  /// activity, project_type_id 6 — both survival-monitoring forms share the
  /// same queue `type` string, so this is how the two pages tell their
  /// pending items apart). Only activities that have actually synced at
  /// some point (and are therefore in the local cache) are considered valid
  /// targets.
  Future<void> _loadPendingItems() async {
    try {
      ...
      final cachedActivities = await OfflineSyncService.getCachedActivities(
