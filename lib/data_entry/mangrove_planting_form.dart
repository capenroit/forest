...
class _MangrovePlantingFormState extends State<MangrovePlantingForm> {
  ...
  /// [_buildMangrovePlantingPhotoName] (the storage object path, which must
  /// stay unique), this is the plain `mangrove_{activityName}_Point{N}` the
  ...
  String _buildMangrovePlantingDisplayName({
    ...
    return 'mangrove_${safeActivityName}_Point$photoIndex$fallbackExtension';
  }
  ...
  Future<void> _handleSave() async {
    ...
    try {
      ...
      Map<String, dynamic> savedMangrovePlanting;
      try {
        ...
        savedMangrovePlanting = await (() async {
          ...
          if (activityId != null && capturedCoordinates.isNotEmpty) {
            ...
            for (final coord in capturedCoordinates) {
              ...
              if (photoPath != null && photoPath.isNotEmpty) {
                ...
                photoCounter++;
                final displayName = _buildMangrovePlantingDisplayName(
                  activityName: activityName,
                ...
                final supabasePhotoUrl =
                    await _uploadMangrovePlantingPhotoToSupabase(
                  localPath: photoPath,
                  photoName: photoName,
                );
      ...
      try {
        savedPlanting = TreePlanting.fromJson(savedMangrovePlanting);
      } catch (e, st) {
        debugPrint(
          'SAVE FAILED (response parse only, save succeeded): $e\n$st',
        );
        savedPlanting = TreePlanting(
          id: (savedMangrovePlanting['id'] ?? planting.id)?.toString(),
          seqId: _toInt(savedMangrovePlanting['seq_id']) ?? planting.seqId,
  ...
  Future<void> _editSeedling(int index) async {
    ...
    setState(() {
      _seedlings[index] =
          SeedlingEntry(seedlingType: species, quantity: quantity);
    });
  }
      ...
      final loadedCoordinates = rows
          .map((row) {
            ...
            return <String, dynamic>{
              ...
              if (locationId != null &&
                  photoUrlByLocationId[locationId] != null)
